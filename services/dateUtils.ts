...
// Mocking the "DATAS" tab logic
// In a real app, this would fetch from the sheet.
// CÉLULA B1: DATA HOJE
// CÉLULA B2: DATA AMANHÃ
export const getDateReference = (): DateReference => {
  ...
  limit.setDate(limit.getDate() + 2); // Example limit
...
export const calculateStatus = (item: PendingItem, refs: DateReference): 'FORA_PRAZO' | 'PRIORIDADE' | 'VENCE_AMANHA' | 'NO_PRAZO' => {
  ...
  if (!limitDate) return 'NO_PRAZO'; // Fallback
