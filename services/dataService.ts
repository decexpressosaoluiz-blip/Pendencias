import { PendingItem, Note, User, UserRole, StalledItem } from '../types';
...
const INITIAL_ADMIN: User = {
  ...
  password: '123', 
  role: UserRole.ADMIN,
  unitDestination: 'ADMIN',
  unitOrigin: 'ADMIN'
};

const MATRIZ_USER: User = {
  id: 'matriz',
  username: 'matriz',
  password: '123',
  role: UserRole.USER,
  unitDestination: 'DEC - MATRIZ',
  unitOrigin: 'DEC - MATRIZ'
}
...
export const initializeData = () => {
  try {
    ...
    if (!users) {
      // Seed Admin and Matriz user
      const adminWithPass = { ...INITIAL_ADMIN, password: '02965740155' };
      localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify([adminWithPass, MATRIZ_USER]));
    }
  } catch (e) {
...
export const getInboxNotes = (originUnit: string): Note[] => {
  ...
  // Return notes where the item belongs to originUnit (as Coleta)
...
export const fetchPendingItems = async (): Promise<{ items: PendingItem[], uniqueUnits: string[], stalledItems: StalledItem[] }> => {
  try {
    ...
    // Safety check for empty or invalid text response
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // Calculate Limit Date: D + E
      let dataLimiteStr = cols[5]; // Default from CSV
      ...
      // Collect Units
      if (cols[7]) uniqueUnits.add(cols[7]); // Coleta
      if (cols[8]) uniqueUnits.add(cols[8]); // Entrega

      // Check notes
      const itemNotes = notes.filter(n => n.cte === cte && n.serie === serie);
      const noteCount = itemNotes ? itemNotes.length : 0;
      ...
      const item: PendingItem = {
        ...
        status: cols[6],
        coleta: cols[7], // Origin
        entrega: cols[8], // Destination
        valorCte: Number(cols[9]),
        txEntrega: Number(cols[10]),
        volumes: Number(cols[11]),
        peso: Number(cols[12]),
        fretePago: cols[13], // Payment Type
      ...
      // Check for Stalled Merchandise (Mercadoria Parada) for Admin
      if (isStalled(item)) {
